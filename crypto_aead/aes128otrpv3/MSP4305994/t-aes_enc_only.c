...
int KeySchedule(int skeylen, int mode, const unsigned char *skey)
{
	...
	{
		case 128:
			AES256_setCipherKey(AES256_BASE, skey, AES256_KEYLENGTH_128BIT);
			break;
		case 192:
			AES256_setCipherKey(AES256_BASE, skey, AES256_KEYLENGTH_192BIT);
			break;
		case 256:
			AES256_setCipherKey(AES256_BASE, skey, AES256_KEYLENGTH_256BIT);
			break;
		default:
			/* key-length error */
...
int Encryption(int skeylen, unsigned char *plain, unsigned char *cipher)
{
	...
	AES256_encryptData(AES256_BASE, plain, cipher);
