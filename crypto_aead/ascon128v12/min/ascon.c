...
int crypto_aead_encrypt(
  ...
  u8 S[SIZE];
  u8 A[s * RATE];  // same as encryption, dynamically inline
  u8 M[t * RATE]; // use max size
  i64 i, j;
...
int crypto_aead_decrypt(
  ...
  *mlen = 0;
  ...
  u8 S[SIZE];
  u8 A[s * RATE]; // replace with inline
  u8 M[t * RATE]; // repalce with inline
  i64 i, j;
  ...
  // return plaintext
  *mlen = clen - klen;
  for (i = 0; i < *mlen; ++i)  // use m the whole time
    m[i] = M[i]; // no need
