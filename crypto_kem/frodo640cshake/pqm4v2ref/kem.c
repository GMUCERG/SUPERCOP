...
int PQCLEAN_FRODOKEM640SHAKE_CLEAN_crypto_kem_keypair(unsigned char *pk, uint8_t *sk) {
    ...
    unsigned char *pk_seedA = &pk[0];
    unsigned char *pk_b = &pk[BYTES_SEED_A];
    unsigned char *sk_s = &sk[0];
    unsigned char *sk_pk = &sk[CRYPTO_BYTES];
    unsigned char *sk_S = &sk[CRYPTO_BYTES + CRYPTO_PUBLICKEYBYTES];
    unsigned char *sk_pkh = &sk[CRYPTO_BYTES + CRYPTO_PUBLICKEYBYTES + 2 * PARAMS_N * PARAMS_NBAR];
    uint16_t B[PARAMS_N * PARAMS_NBAR] = {0};
    uint16_t S[2 * PARAMS_N * PARAMS_NBAR] = {0};           // contains secret data
    uint16_t *E = &S[PARAMS_N * PARAMS_NBAR];               // contains secret data
    unsigned char randomness[2 * CRYPTO_BYTES + BYTES_SEED_A];    // contains secret data via randomness_s and randomness_seedSE
    unsigned char *randomness_s = &randomness[0];                 // contains secret data
    unsigned char *randomness_seedSE = &randomness[CRYPTO_BYTES]; // contains secret data
    unsigned char *randomness_z = &randomness[2 * CRYPTO_BYTES];
    unsigned char shake_input_seedSE[1 + CRYPTO_BYTES];           // contains secret data
    ...
    shake((unsigned char *)S, 2 * PARAMS_N * PARAMS_NBAR * sizeof(uint16_t), shake_input_seedSE, 1 + CRYPTO_BYTES);
    ...
    // Cleanup:
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)S, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)E, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
...
int PQCLEAN_FRODOKEM640SHAKE_CLEAN_crypto_kem_enc(unsigned char *ct, uint8_t *ss, const uint8_t *pk) {
    // FrodoKEM's key encapsulation
    const unsigned char *pk_seedA = &pk[0];
    const unsigned char *pk_b = &pk[BYTES_SEED_A];
    unsigned char *ct_c1 = &ct[0];
    unsigned char *ct_c2 = &ct[(PARAMS_LOGQ * PARAMS_N * PARAMS_NBAR) / 8];
    uint16_t B[PARAMS_N * PARAMS_NBAR] = {0};
    ...
    unsigned char G2in[BYTES_PKHASH + BYTES_MU];                    // contains secret data via mu
    unsigned char *pkh = &G2in[0];
    unsigned char *mu = &G2in[BYTES_PKHASH];                        // contains secret data
    unsigned char G2out[2 * CRYPTO_BYTES];                          // contains secret data
    unsigned char *seedSE = &G2out[0];                              // contains secret data
    unsigned char *k = &G2out[CRYPTO_BYTES];                        // contains secret data
    unsigned char Fin[CRYPTO_CIPHERTEXTBYTES + CRYPTO_BYTES];       // contains secret data via Fin_k
    unsigned char *Fin_ct = &Fin[0];
    unsigned char *Fin_k = &Fin[CRYPTO_CIPHERTEXTBYTES];            // contains secret data
    unsigned char shake_input_seedSE[1 + CRYPTO_BYTES];             // contains secret data
    ...
    shake((unsigned char *)Sp, (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR * sizeof(uint16_t), shake_input_seedSE, 1 + CRYPTO_BYTES);
    ...
    // Cleanup:
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)V, PARAMS_NBAR * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)Sp, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)Ep, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)Epp, PARAMS_NBAR * PARAMS_NBAR * sizeof(uint16_t));
...
int PQCLEAN_FRODOKEM640SHAKE_CLEAN_crypto_kem_dec(unsigned char *ss, const uint8_t *ct, const uint8_t *sk) {
    ...
    uint16_t *Ep = &Sp[PARAMS_N * PARAMS_NBAR];                  // contains secret data
    uint16_t *Epp = &Sp[2 * PARAMS_N * PARAMS_NBAR];             // contains secret data
    const unsigned char *ct_c1 = &ct[0];
    const unsigned char *ct_c2 = &ct[(PARAMS_LOGQ * PARAMS_N * PARAMS_NBAR) / 8];
    const unsigned char *sk_s = &sk[0];
    const unsigned char *sk_pk = &sk[CRYPTO_BYTES];
    const uint16_t *sk_S = (uint16_t *) &sk[CRYPTO_BYTES + CRYPTO_PUBLICKEYBYTES];
    uint16_t S[PARAMS_N * PARAMS_NBAR];                      // contains secret data
    const unsigned char *sk_pkh = &sk[CRYPTO_BYTES + CRYPTO_PUBLICKEYBYTES + 2 * PARAMS_N * PARAMS_NBAR];
    const unsigned char *pk_seedA = &sk_pk[0];
    const unsigned char *pk_b = &sk_pk[BYTES_SEED_A];
    unsigned char G2in[BYTES_PKHASH + BYTES_MU];                   // contains secret data via muprime
    unsigned char *pkh = &G2in[0];
    unsigned char *muprime = &G2in[BYTES_PKHASH];                  // contains secret data
    unsigned char G2out[2 * CRYPTO_BYTES];                         // contains secret data
    unsigned char *seedSEprime = &G2out[0];                        // contains secret data
    unsigned char *kprime = &G2out[CRYPTO_BYTES];                  // contains secret data
    unsigned char Fin[CRYPTO_CIPHERTEXTBYTES + CRYPTO_BYTES];      // contains secret data via Fin_k
    unsigned char *Fin_ct = &Fin[0];
    unsigned char *Fin_k = &Fin[CRYPTO_CIPHERTEXTBYTES];           // contains secret data
    unsigned char shake_input_seedSEprime[1 + CRYPTO_BYTES];       // contains secret data
    ...
    shake((unsigned char *)Sp, (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR * sizeof(uint16_t), shake_input_seedSEprime, 1 + CRYPTO_BYTES);
    for (size_t i = 0; i < (2 * PARAMS_N + PARAMS_NBAR) * PARAMS_NBAR; i++) {
        Sp[i] = PQCLEAN_FRODOKEM640SHAKE_CLEAN_LE_TO_UINT16(Sp[i]);
    }
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_sample_n(Sp, PARAMS_N * PARAMS_NBAR);
    ...
    // Cleanup:
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)W, PARAMS_NBAR * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)Sp, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)S, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)Ep, PARAMS_N * PARAMS_NBAR * sizeof(uint16_t));
    PQCLEAN_FRODOKEM640SHAKE_CLEAN_clear_bytes((unsigned char *)Epp, PARAMS_NBAR * PARAMS_NBAR * sizeof(uint16_t));
