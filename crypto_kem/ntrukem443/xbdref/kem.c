    ...
    const unsigned char *pk)
{
    ...
    uint16_t buf[param->padN*6];
    unsigned char shared_secret[crypto_kem_BYTES + LENGTH_OF_HASH];
    ...
    memset(buf,0, sizeof(uint16_t)*param->padN*6);
    memset(shared_secret, 0, crypto_kem_BYTES + LENGTH_OF_HASH);
    memset(ss, 0, crypto_kem_BYTES);

    /* randomly generate a string to be encapsulated */
    randombytes(shared_secret, crypto_kem_BYTES);
    unpack_public_key(pk,param, h);
    encrypt_cca(cpoly, (char*) shared_secret, crypto_kem_BYTES, h,  buf, param);
    pack_public_key (ct, param, cpoly);

    /* ss = Hash (shared_secret | h) */
    sha512(shared_secret + crypto_kem_BYTES, (unsigned char*)h, sizeof(uint16_t)*param->padN);
    sha512(shared_secret, shared_secret, LENGTH_OF_HASH + crypto_kem_BYTES);
    memcpy (ss, shared_secret, crypto_kem_BYTES);
